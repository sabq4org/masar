    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-5">
        {[
          ...
          ["متأخرة", data.totals.overdue, "text-red-600"],
          ["تستحق اليوم", data.totals.dueToday, "text-amber-700"],
          ["أُنجز آخر ٧ أيام", data.totals.done7d, "text-emerald-700"],
        ].map(([label, value, cls]) => (
          <div key={label as string} className="rounded-xl border border-line bg-white p-4 text-center">
          ...
          {data.byStatus
            ...
            .map((s) => (
              <div key={s.id} className="flex items-center gap-3">
                ...
                <div className="h-3.5 flex-1 overflow-hidden rounded-full bg-line-soft">
                  <div
              ...
              <div key={d.id}>
                <div className="mb-1 flex items-baseline justify-between text-xs">
                  <span className="flex items-center gap-1.5 font-semibold">
                    ...
                    {d.nameAr}
                    {d.overdue > 0 && (
                      <span className="font-bold text-red-600">· {d.overdue} متأخرة</span>
                    )}
                  </span>
            ...
            {data.byUser.map((u) => (
              <div key={u.id} className="flex items-center gap-2.5">
                ...
                <div className="h-3 flex-1 overflow-hidden rounded-full bg-line-soft">
                  <div
                    className={u.overdue > 0 ? "h-full rounded-full bg-red-400" : "h-full rounded-full bg-accent"}
                    style={{ width: `${(u.open / maxUser) * 100}%` }}
                  />
                </div>
                <span className="w-10 flex-none text-left text-xs tabular-nums">
                  {u.open}
                  {u.overdue > 0 && <span className="text-red-600"> ({u.overdue})</span>}
                </span>
              </div>
            ))}
